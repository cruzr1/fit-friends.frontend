...
type PersonalAccountComponentProps = {
  user: UserType;
}

export default function PersonalAccountComponent({user}: PersonalAccountComponentProps): JSX.Element {
  const dispatch = useAppDispatch();
  const {name, description, isReadyTrain, trainType, location, role, gender, level, avatar, caloriesDaily = 0, caloriesTarget = 0, certificates} = user;
  const coachCertificates = certificates ? [certificates] : [];
  ...
  const handleTrainTypeChange = (type: TrainType) => {
    const newTrainType = userTrainType.includes(type)
      ? userTrainType.filter((trainingType) => trainingType !== type)
          ...
          <section className="user-info">
            <div className="user-info__header">
              <div className="input-load-avatar">
                <label>
                  <input className="visually-hidden" type="file" name={avatar} accept="image/png, image/jpeg" /><span className="input-load-avatar__avatar"><img src={`img/content/avatars/users/${adaptImage(avatar)}.png`} srcSet={`img/content/avatars/users/${adaptImage(avatar)}@2x.png 2x`} width="98" height="98" alt="user avatar" /></span>
                </label>
              </div>
            </div>
              ...
              ><span className="custom-select__label">Локация</span>
                ...
                <ul className="custom-select__list" role="listbox" onClick={(evt) => handleLocationItemChange(evt.target as HTMLUListElement)}>
                  {Object.values(Location).map((locationItem) =>
                    <li key={locationItem} className="custom-select__item" id={locationItem} aria-selected={locationItem === userLocation} role='option'>{LocationCaption[locationItem]}</li>)}
                </ul>
              </div>
              <div className={`
                  custom-select
              ...
              ><span className="custom-select__label">Пол</span>
                ...
                <ul className="custom-select__list" role="listbox" onClick={(evt) => handleGenderItemChange(evt.target as HTMLUListElement)}>
                  {Object.values(Gender).map((genderItem) =>
                    <li key={genderItem} className="custom-select__item" id={genderItem} aria-selected={userGender === genderItem} role='option'>{GenderCaption[genderItem]}</li>)}
                </ul>
              </div>
              <div className={`
                  custom-select
              ...
              ><span className="custom-select__label">Уровень</span>
                ...
                <ul className="custom-select__list" role="listbox" onClick={(evt) => handleLevelItemChange(evt.target as HTMLUListElement)}>
                  {Object.values(Level).map((option) =>
                    <li key={option} className="custom-select__item" id={option} aria-selected={userLevel === option} role='option'>{LevelCaption[option]}</li>)}
                </ul>
              </div>
            </form>
          </section>
          <div className="inner-page__content">
            ...
            {isCoach &&
              <div className="personal-account-coach">
                ...
                <CoachCertificatesComponent certificates={coachCertificates}/>
              </div>}
          </div>
        </div>
      </div>
