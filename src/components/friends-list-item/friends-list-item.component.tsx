...
export default function FriendsListItemComponent({userId, name, location, trainType, isReadyTrain, role, userRole, application}: FriendsListItemProps): JSX.Element {
  ...
  `;
  const handleRejectButtonClick = (evt: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    evt.preventDefault();
    if (application) {
      dispatch(updateApplicationAction({
  ...
  const handleAcceptButtonClick = (evt: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    evt.preventDefault();
    if (application) {
      dispatch(updateApplicationAction({
    ...
    <div className="thumbnail-friend" data-testid='friend'>
      <div className={`thumbnail-friend__info thumbnail-friend__info--theme${isFriendTrainer ? '-dark' : '-light'}`}>
        ...
        <div className="thumbnail-friend__activity-bar">
          ...
          {!isTrainer && isReadyTrain && !isFriendTrainer &&
